...
def market_summary() -> str:
    top_by_value = rank_credits(MARKETPLACE_CREDITS)[:3]
    top_selling = sorted(MARKETPLACE_CREDITS, key=lambda c: c["demand_score"], reverse=True)[:3]

    def credit_line(c):
        seller = SELLER_PROFILES.get(c["seller_id"], {})
        return (
...
def answer_market_question(user_input: str) -> str:
