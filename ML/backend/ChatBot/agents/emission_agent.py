...
def find_credit_by_id(credit_id: str):
    for credit in MARKETPLACE_CREDITS:
...
def answer_emission_question(user_input: str) -> str:
    quantity = extract_quantity(user_input)
    credit = None
    for c in MARKETPLACE_CREDITS:
    ...
    if not credit:
        ...
        credit = max(MARKETPLACE_CREDITS, key=lambda c: c["emissions_offset_tons"])
    ...
    prompt = (
        ...
        "Emissions context:\n" + context +
        "\n\nExplain the calculation in plain language and note assumptions."
    )
